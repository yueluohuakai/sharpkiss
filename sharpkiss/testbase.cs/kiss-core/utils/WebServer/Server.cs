...
namespace Kiss.Utils.Server
{
    public class Server : MarshalByRefObject
    {
        ...
        Host GetHost()
        {
            ...
            if (host == null)
            {
                lock (this)
                {
                    ...
                    if (host == null)
                    {
                        host = (Host)CreateWorkerAppDomainWithHost(_virtualPath, _physicalPath, _port, typeof(Host));
        ...
        static object CreateWorkerAppDomainWithHost(string virtualPath, string physicalPath, int port, Type hostType)
        {
            // this creates worker app domain in a way that host doesn't need to be in GAC or bin
            // using BuildManagerHost via private reflection
            string uniqueAppString = string.Concat(virtualPath, string.Concat(physicalPath, port)).ToLowerInvariant();
