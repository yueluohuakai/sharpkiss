...
//+ History:
//+-------------------------------------------------------------------+
//+ 2009/05/07		zhli Comment Created
//+-------------------------------------------------------------------+
#endregion
...
namespace Kiss.Utils
{
    ...
    {
        public static T Load<T> ( string str , string ns )
        {
            XmlSerializer ms = new XmlSerializer ( typeof ( T ) , ns );
            using ( StringReader rdr = new StringReader ( str ) )
            {
                using ( XmlReader xr = XmlReader.Create ( rdr ) )
                {
        ...
        public static string Save<T> ( T obj , string ns )
        {
            XmlSerializer ms = new XmlSerializer ( typeof ( T ) , ns );

            using ( StringWriter writer = new StringWriter ( ) )
            {
                ms.Serialize ( writer , obj );

                return writer.GetStringBuilder ( ).ToString ( );
            }
        }

        public static string Save ( object obj , string ns )
        {
            XmlSerializer ms = new XmlSerializer ( obj.GetType ( ) , ns );
            using ( StringWriter writer = new StringWriter ( ) )
            {
                ms.Serialize ( writer , obj );
                return writer.GetStringBuilder ( ).ToString ( );
        ...
        /// Converts a string of xml to the supplied object type. 
        /// </summary>
        /// <param name="node">Xml representing a .NET object</param>
        /// <param name="objectType">The type of object which the xml represents</param>
        /// <returns>A instance of object or null if the value of xml is null or empty</returns>
